        ...
        )
{
    ...
    if (nrhs != 1) {
        mexErrMsgIdAndTxt( "MATLAB:mxsetnzmax:invalidNumInputs",
                "One input argument required.");
    }
    if(nlhs > 1){
        mexErrMsgIdAndTxt( "MATLAB:mxsetnzmax:maxlhs",
                "Too many output arguments.");
    }
    
    if(!mxIsSparse(prhs[0])) {
        mexErrMsgIdAndTxt( "MATLAB:mxsetnzmax:invalidInputSparisty",
    ...
    if(mxGetNzmax(plhs[0]) == actual_number_of_non_zeros) {
        mexWarnMsgIdAndTxt( "MATLAB:mxsetnzmax:badNumNonzeros", 
                "The actual number of non-zeros is already equal to the non-zero maximum for this sparse matrix.\n");
    }
    else {
        void *newptr;
        mwIndex *ir;
        size_t nbytes;
            ...
            if(mxIsComplex(plhs[0])) {
                complexPtr = mxGetComplexDoubles(plhs[0]);
                nbytes = actual_number_of_non_zeros * sizeof(*complexPtr);
                newptr = mxRealloc(complexPtr, nbytes);
                mxSetComplexDoubles(plhs[0], newptr);
            }
            else {
                ptr = mxGetDoubles(plhs[0]);
                nbytes = actual_number_of_non_zeros * sizeof(*ptr);
                newptr = mxRealloc(ptr, nbytes);
                mxSetDoubles(plhs[0], newptr);
            }
        #else
            nbytes = actual_number_of_non_zeros * sizeof(*ptr);
            ptr = mxGetPr(plhs[0]);
