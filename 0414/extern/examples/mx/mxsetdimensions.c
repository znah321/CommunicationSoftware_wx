 ...
 * mxsetdimensions.c 
 *
 * mxsetdimensions reshapes your input array according to the new
...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
    ...
    if (mxIsSparse(prhs[0]) && number_new_dims != 2){
        mexErrMsgIdAndTxt( "MATLAB:mxsetdimensions:invalidInput",
                "Multidimensional sparse arrays are not supported.\n");
    }
    ...
    for (i=0; i< number_new_dims;i++){
        ...
        if(mxGetNumberOfElements(pa) != 1) {
            ...
            mxFree(new_dims);
            mexErrMsgIdAndTxt( "MATLAB:mxsetdimensions:invalidSize",
    ...
    if (number_new_elements != number_input_elements){
        /* Free allocated memory */
        mxFree(new_dims);
        mexErrMsgIdAndTxt( "MATLAB:mxsetdimensions:invalidNumelements",
                "Total number of elements in the new array, must equal number of elements in input array.\n");
    }
    /* Duplicate the array */
