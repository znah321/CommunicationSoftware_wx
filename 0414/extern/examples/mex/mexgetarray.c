...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    ...
    if (nrhs != 0) {
        mexErrMsgIdAndTxt("MATLAB:mexgetarray:minrhs", "No input arguments required.");
    }
    if (nlhs > 1) {
        mexErrMsgIdAndTxt("MATLAB:mexgetarray:maxrhs", "Too many output arguments.");
    }
    ...
    mxGetDoubles(array_ptr)[0] += 1;
    mex_count = (int)mxGetDoubles(array_ptr)[0];
#else
